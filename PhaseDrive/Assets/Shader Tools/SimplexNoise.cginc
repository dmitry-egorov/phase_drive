						...
						out float4 hash_2	)		//	generates 3 random numbers for each of the 4 3D cell corners.  cell corners:  v0=0,0,0  v3=1,1,1  the other two are user definable
{
	...
	static const float2 OFFSET = float2( 50.0, 161.0 );
	static const float DOMAIN = 69.0;
	static const float3 SOMELARGEFLOATS = float3( 635.298681, 682.357502, 668.926525 );
	static const float3 ZINC = float3( 48.500388, 65.294118, 63.934599 );
									...
									out float4 v1234_z )
{
	...
	static const float SKEWFACTOR = 1.0/3.0;
	static const float UNSKEWFACTOR = 1.0/6.0;
	static const float SIMPLEX_CORNER_POS = 0.5;
	static const float SIMPLEX_PYRAMID_HEIGHT = 0.70710678118654752440084436210485;	// sqrt( 0.5 )	height of simplex pyramid.
...
float simplex(float3 p)
{
	...
	static const float FINAL_NORMALIZATION = 37.837227241611314102871574478976;
...
float simplex_fbm(float3 p, int octaves, float lacunarity, float persistence)
{
	float sum = 0;
	float a = 1;
	float f = 1;
